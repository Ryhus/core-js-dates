...
function getNextFriday(date) {
  const nubmberOfDays = 7;
  const indexOfWantedWeekDay = 5;
  const currentWeekDay = date.getDay();
  const currentDate = date.getDate();

  if (currentWeekDay < indexOfWantedWeekDay) {
    return new Date(
      date.setDate(
        currentDate + indexOfWantedWeekDay - (currentWeekDay % nubmberOfDays)
      )
    );
  }
  ...
  return new Date(
    date.setDate(
      currentDate + nubmberOfDays - (currentWeekDay % indexOfWantedWeekDay)
    )
  );
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const milisecondsPerDay = 86400000;
  return (new Date(dateEnd) - new Date(dateStart)) / milisecondsPerDay + 1;
}
...
function isDateInPeriod(date, period) {
  const currentDateTimestamp = new Date(date).getTime();
  const startPeriodTimestamp = new Date(period.start).getTime();
  const endPeriodTimestamp = new Date(new Date(period.end).getTime());

  return (
    currentDateTimestamp >= startPeriodTimestamp &&
    currentDateTimestamp <= endPeriodTimestamp
  );
}
...
function getCountWeekendsInMonth(month, year) {
  const currentMonth = new Date(year, month - 1);
  const nextMonth = new Date(year, month);
  const numberOfDays = Math.floor((nextMonth - currentMonth) / 86400000);
  let numberOfWeekendDays = 0;
  for (let i = 0; i < numberOfDays; i += 1) {
    const currentWeekDay = currentMonth.getDay();
...
function getWeekNumberByDate(date) {
  ...
  let weekNumber = 0;
  while (endDate >= startDate.getTime()) {
    if (startDate.getDay() === startWeekDay) {
      weekNumber += 1;
    }
...
function getNextFridayThe13th(date) {
  ...
  while (startDate.getDay() !== 5 || startDate.getDate() !== 13) {
    startDate.setDate(startDate.getDate() + 1);
  }
...
function getQuarter(date) {
  ...
  const quarts = [1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4];
  return quarts[month];
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      workingDays.push(
        startDate.toLocaleDateString('en-GB', options).split('/').join('-')
      );
      startDate.setDate(startDate.getDate() + 1);
